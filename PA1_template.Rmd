...
#mean # of steps
MeanSteps <- mean(activitySumDay$totalsteps)

#median # of steps
MedianSteps <- median(activitySumDay$totalsteps)
```
...
Therefore, the five minute interval that contains the maximum number of steps is `r MaxInterval[,1]`.  
...
So there are `r nrow(ActivityNAdata)` rows that have a NA in the original dataset.
...
```{r echo=FALSE}
#mean # of steps
MeanCompleteSteps <- mean(CompleteSumDS$totalsteps)

#median # of steps
MedianCompleteSteps <- median(CompleteSumDS$totalsteps)
```

The mean total number of steps taken per day is `r MeanCompleteSteps` and the median total number of steps taken per day is `r MedianCompleteSteps`.  

As a resut of replacing NA values as opposed to removing them altogether with this dataset, the mean and median values are different. When I replace them with the median value of steps for that interval, and then calculate mean and median again, my values are smaller.   In Part 1 where I just removed NA rows completely before calculating mean my mean was `r MeanSteps` and when replacing NA's with the median interval value, my new mean is `r MeanCompleteSteps`.  The same is true for the median values.  In part 1, I had a median value of `r MedianSteps` and replacing values, I now get a median step of `r MedianCompleteSteps`. 
